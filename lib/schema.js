...
export const SCHEMA_STATEMENTS = [
  ...
  `CREATE TABLE IF NOT EXISTS users (
    ...
    username TEXT UNIQUE NOT NULL,
    password_hash TEXT NOT NULL,
    role TEXT NOT NULL CHECK (role IN ('admin', 'sales', 'dataentry')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  `CREATE TABLE IF NOT EXISTS fresh_leads (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
export const MIGRATIONS = [
  ...
  // Allow the new 'dataentry' role on existing DBs (the original CHECK
  // only had 'admin','sales'). Idempotent: drop + recreate the constraint.
  `ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check`,
  `ALTER TABLE users ADD CONSTRAINT users_role_check
     CHECK (role IN ('admin', 'sales', 'dataentry'))`,
];
