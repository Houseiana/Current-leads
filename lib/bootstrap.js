...
// Parse "user1:pass1,user2:pass2" into [{ username, password }]. Used for
// both EXTRA_SALES_USERS and EXTRA_ADMIN_USERS.
function parseUserPairs(raw) {
  if (!raw) return [];
  return raw
    .split(",")
    .map((pair) => pair.trim())
...
async function doBootstrap() {
  ...
  const updated = [];
  for (const u of parseUserPairs(process.env.EXTRA_SALES_USERS)) {
  ...
  for (const u of parseUserPairs(process.env.EXTRA_ADMIN_USERS)) {
    const r = await upsertUser(u.username, u.password, "admin");
    if (r === "inserted") seeded.push(u.username);
    else if (r === "updated") updated.push(u.username);
  }
  for (const u of parseUserPairs(process.env.EXTRA_DATAENTRY_USERS)) {
    const r = await upsertUser(u.username, u.password, "dataentry");
    if (r === "inserted") seeded.push(u.username);
    else if (r === "updated") updated.push(u.username);
  }
...
export async function bootstrapStatus() {
  const status = {
    env: {
      ...
      EXTRA_ADMIN_USERS: !!process.env.EXTRA_ADMIN_USERS,
      EXTRA_DATAENTRY_USERS: !!process.env.EXTRA_DATAENTRY_USERS,
    },
